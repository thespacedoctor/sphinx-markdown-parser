...
class AutoStructify(transforms.Transform):
    ...
    def auto_toc_tree(self, node):  # pylint: disable=too-many-branches
        ...
        return self.state_machine.run_directive(
            ...
            content=['%s <%s>' % (k, v.replace(".html", "")) for k, v in refs]
        )
    ...
    def auto_code_block(self, node):
        ...
        if 'language' not in node and len(node.children) and isinstance(node.children[0], nodes.literal) and 'class' not in node.children[0]:
            return None

        if 'language' not in node:
            lang = node.children[0]["class"]
            node = node.children[0]
            original_node = node
            node["language"] = lang

        if not hasattr(self.document.settings, 'tab_width'):
            self.document.settings.tab_width = 4

        self.state_machine.reset(
            self.document, node, self.current_level
        )
